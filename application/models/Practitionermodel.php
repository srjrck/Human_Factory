...
class PractitionerModel extends CI_model {
    ...
    function get_practitioner($id) {
        $response = [];
        $this->db->select("id,type");
        $this->db->from("resource");
        $this->db->where('id', $id);
        $this->db->where('type', 'Practitioner');
        ...
        if (isset($result) && count($result) > 0) {
            ...
            $response['issue']['details']['text'] = "No such practitioner with id: $id";
        }
        return $response;
    }

    function check_practitioner($id) {
        $this->db->select("id,type");
        $this->db->from("resource");
        $this->db->where('id', $id);
        $this->db->where('type', 'Practitioner');
    ...
    function get_practitioner_list() {
        $data = [];
        $identifier_data = [];
        $name_data = [];
        $telecom_data = [];
        $query = $this->db->get_where('resource', array('type' => 'Practitioner', 'is_removed' => 'false'));
        $result = $query->result();
        if (isset($result) && count($result)>0) {
    ...
    function create_practitioner($resouceData, $identifierData, $nameData, $telecomData) {
        if ($resouceData) {
            $found = false;
            ...
            if (!$found) {
                ...
                $response['issue']['details']['text'] = "Practitioner created successfully";
            } else {
                http_response_code(409);
                $response['resourcetype'] = "OperationOutcome";
                $response['issue']['severity'] = "error";
                $response['issue']['code'] = "conflict";
                $response['issue']['details']['text'] = "Practitioner with identifiers already exists";
            }
        } else {
    ...
    function delete_practitioner($resource_id) {
        if ($resource_id) {
            $this->db->select("*");
            $this->db->from("resource");
            ...
            if (isset($result) && count($result) > 0) {
                ...
                if ($this->db->affected_rows() > 0) {
                    ...
                    $response['resourcetype'] = "OperationOutcome";
                    $response['issue']['severity'] = "success";
                    $response['issue']['code'] = "removed";
                    $response['issue']['details']['text'] = "Practitioner removed successfully";
                } else {
                    http_response_code(400);
                    $response['resourcetype'] = "OperationOutcome";
                    $response['issue']['severity'] = "error";
                ...
                $response['issue']['severity'] = "error";
                $response['issue']['code'] = "notfound";
                $response['issue']['details']['text'] = "No practitioner found with id: $resource_id";
            }
        } else {
            http_response_code(400);
            $response['resourcetype'] = "OperationOutcome";
            $response['issue']['severity'] = "error";
            $response['issue']['code'] = "notfound";
            $response['issue']['details']['text'] = "Please provide practitioner id to remove";
        }
        return $response;
    }
