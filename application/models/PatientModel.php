...
class PatientModel extends CI_model {
  ...
  function create_patient($resouceData, $identifierData, $nameData, $telecomData) {
    if ($resouceData) {
      ...
      if (!$found) {
        ...
        $response['issue']['details']['text'] = "Patient with identifiers already exists";
      }
    } else {
  ...
  function get_patient($id){
    ...
    if (isset($result) && count($result) > 0) {
      ...
      $response['issue']['details']['text'] = "No such patient with id: $id";
    }
    return $response;
  }
    ...
    if ($resource_id) {
      ...
      if (isset($result) && count($result) > 0) {
        ...
        if ($this->db->affected_rows() > 0) {
          ...
          $response['issue']['details']['text'] = "patient removed successfully";
        } else {
          http_response_code(400);
          $response['resourcetype'] = "OperationOutcome";
          $response['issue']['severity'] = "error";
        ...
        $response['issue']['severity'] = "error";
        $response['issue']['code'] = "notfound";
        $response['issue']['details']['text'] = "No patient found with id: $resource_id";
      }
    } else {
      http_response_code(400);
      $response['resourcetype'] = "OperationOutcome";
      $response['issue']['severity'] = "error";
      $response['issue']['code'] = "notfound";
      $response['issue']['details']['text'] = "Please provide patient id to remove";
    }
    return $response;
  }
